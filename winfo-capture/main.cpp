...
obs_source_t* create_source() {
	return obs_source_create("window_capture", "window_capture",
					NULL, nullptr);
}
...
void main() {
	...
	init();
	SetProcessDpiAwareness(PROCESS_SYSTEM_DPI_AWARE);//在系统设置缩放比的情况下，GetClientRect获取到正确的rect大小，以完全抓取窗口
